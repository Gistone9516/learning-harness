...
실제 _예시 목업 콘텐츠로 부트 -> 큐 -> 채점 -> 전이 -> 영속 왕복 전 체인 검증.
_예시 원본 오염 방지를 위해 임시 폴더로 복사해 그 위에서 부트한다.
"""
...
_EXAMPLE = os.path.join(os.path.dirname(_BOT_ROOT), "_예시")


def _mount_copy():
    tmp = tempfile.mkdtemp(prefix="clf_ex_")
    dst = os.path.join(tmp, "예시")
...
def test_boot_example_loads():
    ...
    try:
        ...
        assert len(r.enabled_capabilities) >= 1            # 미지정 -> core 전체
    finally:
...
def test_score_exact_card_correct():
    ...
    try:
        r = boot.load(mount)
        # 실효 grade_mode가 exact인 카드 하나 선택
        card = next(
