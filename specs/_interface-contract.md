...
1. **Subject-agnostic** — no subject vocabulary, content, or hardcoded thresholds in engine, harness, or bot. Area taxonomy, AI persona, AI task wording, and all subject UI strings are injected via config (`bot/subject.py` SubjectProfile; pass_targets and all others likewise). The kit ships only generic, subject-neutral defaults. Enforced by `bot/tests/test_subject_agnostic.py` (kit code must contain zero subject literal).
...
11. **Canonical kit, mount model (one app + per-subject data)** — this repo is the canonical, generic code and is never customized per subject. There is exactly ONE code copy: run `bot/main.py` from the kit and pass a **data-only subject folder** (manifest, decks, config, `_state`) as the mount. Each subject folder also holds its own `.env`; run from that folder so its channel/token load (`load_dotenv()` reads the cwd). **No per-subject code clone or fork** (the former `tools/clone.py` is removed). Kit changes live only in this repo and apply to every subject at once; the subject's data is never modified by the kit.
...
13. **UI = Components V2 only** — learner-facing content is rendered with Components V2 layout cards (`harness/output/cards.py`: `card`, `titled_card`, `container`); `discord.Embed` and `output/embeds.py` are deprecated and not used by the kit's output (catalog-only). Plain text is allowed only for short ephemeral acknowledgements and for `@mention` pings, because a V2 card cannot carry message content or a mention, so the ping is a separate plain message.
    - **Tables** — Discord does not render markdown tables (pipes show as raw text), so all tabular content is emitted as a CJK-width-aware monospace code-block table via `bot/text_format.py` (`render_table` / `format_tables`). This is the single source of truth for display width and is reused by `harness/live/livetable.py`. Free-form AI/explanation text is passed through `format_tables` so any model-emitted pipe table is normalized.
