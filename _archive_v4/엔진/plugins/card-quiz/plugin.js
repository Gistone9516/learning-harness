...
(function () {
  ...
  function _bootQuiz(extraOpts) {
    ...
    // 좌측 네비에서 선택한 활성 덱(스테이지 전환). 지정되면 그 덱만 출제.
    var activeDeckId = (typeof window !== 'undefined' && window.__CLF_ACTIVE_DECK__) || null;
    var needsCustomBoot = hasUnitFilter || hasDisabledCards || hasRetryList || !!activeDeckId;
    ...
    if (!needsCustomBoot) {
      ...
      try {
        ...
        // 활성 덱이 지정되고 manifest에 존재하면 그 덱으로 전환(스테이지 선택)
        if (activeDeckId) {
          var hasActive = deckIds3.some(function (x) {
            var id = (typeof x === 'string') ? x : (x && x.deck_id);
            return id === activeDeckId;
          });
          if (hasActive) deckId3 = activeDeckId;
        }
