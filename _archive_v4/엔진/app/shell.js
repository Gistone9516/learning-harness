...
(function () {
  ...
  function _renderConcept() {
    ...
    cards.filter(function (c) { return c.enabled !== false; }).forEach(function (card) {
      ...
      var refId = (card.links && card.links.concept_ref) || '';  // card_id 노출 금지
      var title = frontText(card) || '(문항)';
      var head = document.createElement('button'); head.type = 'button'; head.className = 'sec-head';
      head.innerHTML = (refId ? '<span class="sec-id">' + _esc(refId) + '</span>' : '') +
  ...
  function _buildNav(manifest) {
    ...
    areas.forEach(function (a, i) {
      ...
      head.addEventListener('click', function () {
        ...
        // 스테이지(그룹) 펼칠 때 그 스테이지의 첫 덱 퀴즈로 전환
        if (willOpen && areaDecks.length) {
          var f0 = areaDecks[0];
          var fid = (typeof f0 === 'string') ? f0 : (f0 && f0.deck_id);
          if (fid) {
            _conceptDeckId = fid;
            window.__CLF_ACTIVE_DECK__ = fid;
            _activePluginId = null; _activeMountOk = false;
            _setRoute('learn', 'card-quiz', fid);
          }
        }
      });
      ...
      areaDecks.forEach(function (d) {
        ...
        link.addEventListener('click', function () {
          ...
          // 선택한 덱(스테이지)으로 퀴즈 전환: 활성 덱 지정 + 강제 재마운트
          _conceptDeckId = deckIdVal;
          window.__CLF_ACTIVE_DECK__ = deckIdVal;
          _activePluginId = null; _activeMountOk = false;  // 이미 마운트돼 있어도 재부팅
          _setRoute('learn', 'card-quiz', deckIdVal);
        });
